...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const listeningLessons = (lessonProgress || []).filter(
      (p: LessonProgress & { lesson?: { topics?: string[] } }) =>
        p.lesson?.topics?.includes("listening") ||
        p.lesson?.topics?.includes("comprehension")
    ).length;

    const speakingLessons = (lessonProgress || []).filter(
      (p: LessonProgress & { lesson?: { topics?: string[] } }) =>
        p.lesson?.topics?.includes("speaking") ||
        p.lesson?.topics?.includes("pronunciation") ||
        p.lesson?.topics?.includes("conversation")
    ).length;

    const readingLessons = (lessonProgress || []).filter(
      (p: LessonProgress & { lesson?: { topics?: string[] } }) =>
        p.lesson?.topics?.includes("reading")
    ).length;

    const writingLessons = (lessonProgress || []).filter(
      (p: LessonProgress & { lesson?: { topics?: string[] } }) =>
        p.lesson?.topics?.includes("writing")
    ).length;

    const grammarLessons = (lessonProgress || []).filter(
      (p: LessonProgress & { lesson?: { topics?: string[] } }) =>
    ...
    const skills: SkillResponse[] = [
      {
        ...
        percentage: Math.min(
          100,
          Math.max(20, basePercentage + listeningLessons * 5)
        ),
      },
      {
        skill: "Speaking",
        ...
        percentage: Math.min(
          100,
          Math.max(15, basePercentage + speakingLessons * 6)
        ),
      },
      {
        skill: "Reading",
        ...
        percentage: Math.min(
          100,
          Math.max(25, basePercentage + readingLessons * 7)
        ),
      },
      {
        skill: "Writing",
        ...
        percentage: Math.min(
          100,
          Math.max(10, basePercentage + writingLessons * 8)
        ),
      },
      {
        skill: "Grammar",
        ...
        percentage: Math.min(
          100,
          Math.max(20, basePercentage + grammarLessons * 6)
        ),
      },
      {
        skill: "Vocabulary",
        ...
        percentage: Math.min(
          ...
          Math.max(
            30,
            basePercentage + Math.floor((vocabularyCount || 0) / 5) * 3
          )
        ),
      },
    ];
